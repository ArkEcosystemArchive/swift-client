...
extension Two {
    class Transactions {
        ...
        public func search(body: [String: Any]?, completionHandler: @escaping (Dictionary<String, Any>?) -> Void) {
        ...
        public func allUnconfirmed(limit: Int = 20, page: Int = 1, completionHandler: @escaping (Dictionary<String, Any>?) -> Void) {
            apiGetHandler("\(endpoint)/unconfirmed", ["limit": limit, "page": page], completionHandler)
        }
        
        /// Retrieves all transactions types
        public func types(completionHandler: @escaping (Dictionary<String, Any>?) -> Void) {
